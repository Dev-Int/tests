...
### Immuabilité email côté User

Conformément à la règle d'immutabilité documentée ci-dessus, `changeEmail()` a été supprimé de `User` et `$email` est déclaré `readonly` dans le constructeur. Tout changement d'email implique la création d'un nouvel utilisateur.
