...
> **Exception — emailExists()** : La méthode `UserRepository::emailExists()` est volontairement
> non filtrée par `disabledAt`. L'email est un identifiant permanent non recyclable (ADR-008) :
> un email ayant appartenu à un user désactivé reste indisponible définitivement.
>
> **Cas futur non traité — réembauche** : Si le métier doit permettre la réembauche d'un employé
> avec le même email, cela nécessitera : (1) une contrainte UNIQUE partielle en BDD
> (`WHERE disabled_at IS NULL`), (2) un filtre `disabledAt IS NULL` dans `emailExists()`,
> et (3) une stratégie de réactivation du User désactivé plutôt que d'en créer un nouveau.
