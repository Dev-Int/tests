...
**Caractéristiques :**
- Email : Identifiant de connexion (unique, **immuable après création — ADR-008**)
...
security:
    ...
    access_control:
        ...
        - { path: ^/inventories/, roles: ROLE_USER }
...
### Defense-in-depth : deux couches indépendantes

Le projet utilise volontairement une double protection pour les routes sensibles :

| Couche | Mécanisme | Évalué | Comportement |
|--------|-----------|--------|-------------|
| 1 - Firewall | `access_control` dans `security.yaml` | Avant le controller | Redirect login / 403 Symfony |
| 2 - Application | `#[RequireRole]` via `AuthenticationListener` | Au démarrage du controller | Redirect + flash / 403 applicatif |

**Pourquoi les deux ?**
- La couche firewall est un filet de sécurité : si `#[RequireRole]` est oublié sur un nouveau controller, l'accès reste bloqué.
- La couche applicative permet des comportements riches (messages flash, redirect custom, logique Auth BC).
- C'est du defense-in-depth : deux barrières indépendantes.

**Règle** : Ne jamais supprimer une entrée `access_control` sous prétexte que `#[RequireRole]` existe déjà.
