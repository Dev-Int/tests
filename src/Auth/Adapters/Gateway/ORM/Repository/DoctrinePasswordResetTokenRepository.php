...
use Auth\Entities\Exception\TokenNotFound;
...
final class DoctrinePasswordResetTokenRepository extends ServiceEntityRepository implements PasswordResetTokenRepository
{
    ...
    {
        $userOrm = $this->getEntityManager()->find(UserORM::class, $token->user()->uuid()->toString());
        if (!$userOrm instanceof UserORM) {
            throw new UserNotFoundById($token->user()->uuid());
        }

        $resetToken = new PasswordResetTokenORM(
    ...
    public function save(ResetPassword $token): void
    {
        ...
        if (!$existingToken instanceof PasswordResetTokenORM) {
            throw new TokenNotFound($token->token);
        }
    ...
    private function persist(PasswordResetTokenORM $token): void
    {
