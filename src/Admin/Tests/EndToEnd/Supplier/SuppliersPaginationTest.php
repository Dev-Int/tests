...
final class SuppliersPaginationTest extends BasePantherTestCase
{
    ...
    public function testCorrectNumberOfItemsPerPage(): void
    {
        ...
        self::assertCount(
            ...
            $suppliers,
            \sprintf('La première page devrait afficher %d fournisseurs', self::DEFAULT_ITEMS_PER_PAGE)
        );
        ...
        self::assertCount(
            self::DEFAULT_ITEMS_PER_PAGE,
            $suppliers,
            \sprintf('La deuxième page devrait afficher %d fournisseurs', self::DEFAULT_ITEMS_PER_PAGE)
        );
        ...
        self::assertCount(
            self::DEFAULT_ITEMS_PER_PAGE,
            $suppliers,
            \sprintf('La troisième page devrait afficher %d fournisseurs', self::DEFAULT_ITEMS_PER_PAGE)
        );
    }
    ...
    public function testLastPageWithPartialItems(): void
    {
        ...
        $suppliers = $client->getCrawler()->filter('ul.table > turbo-frame[id^="supplier_"]');
        self::assertCount(
            $expectedItemsOnLastPage,
            $suppliers,
            \sprintf(
                'La dernière page devrait afficher %d fournisseurs (%d total / %d par page)',
                $expectedItemsOnLastPage,
                self::SUPPLIERS_WITH_PARTIAL_LAST_PAGE,
                self::DEFAULT_ITEMS_PER_PAGE
            )
        );
    ...
    public function testNextButtonDisabledOnLastPage(): void
    {
        ...
        // Le bouton "Next" devrait avoir la classe "disable-link"
        $crawler = $client->getCrawler();
    ...
    public function testChangeItemsPerPageTo50(): void
    {
        ...
        $crawler = $client->getCrawler();
        $suppliers = $crawler->filter('ul.table > turbo-frame[id^="supplier_"]');
        self::assertCount(self::DEFAULT_ITEMS_PER_PAGE, $suppliers, 'Par défaut, on devrait avoir 25 fournisseurs');
        ...
        self::assertCount(
            50,
            $suppliers,
            'Après avoir sélectionné 50 items par page, on devrait voir 50 fournisseurs'
        );
    ...
    public function testItemsPerPagePersistsAcrossPages(): void
    {
        ...
        self::assertCount(
            25,
            $suppliers,
            'La page 2 devrait afficher les 25 fournisseurs restants'
        );
    }

    private function createSuppliersForPagination(int $count): void
    {
        ...
        // createMinimalConfiguration() crée déjà 1 supplier, on en ajoute (count - 1)
        for ($i = 2; $i <= $count; $i++) {
            $supplier = (new SupplierDataBuilder())->create(
                \sprintf('Fournisseur %03d', $i),
                $familyLog
            )
                ->withUuid($faker->uuid())
                ->build()
