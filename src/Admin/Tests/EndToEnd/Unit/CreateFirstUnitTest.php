...
final class CreateFirstUnitTest extends BasePantherTestCase
{
    public function testCreateFirstUnitSuccessfully(): void
    {
        ...
        $unitLabel = $faker->word();
        $unitAbbreviation = $faker->word();
        ...
        // Vérifier qu'on est redirigé (vers /admin/units)
        $client->wait(2);
        ...
        // Vérifier que l'unité apparaît dans la page (que ce soit la liste ou un message)
        $client->wait(1);
        // La page peut afficher soit la liste des unités, soit un message de succès
        // On vérifie juste qu'on a quitté la page de création
    ...
    public function testCancelDuringFirstUnitCreation(): void
    {
        ...
        // Vérifier que le bouton Cancel est présent
        ...
        // Vérifier la redirection vers /admin/configure (car pas d'unités en base)
        $client->wait(2);
